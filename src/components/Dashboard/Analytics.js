        ...
        feeds.map((feed, i) => {
          return (
            <Box
              ...
              key={i}
              onClick={(e) =>
                handleShowChart(feed.questions, feed.analytics, feed.feedFor)
              }
            >
              {/* <Chart label={feed.} /> */}
        ...
        <DialogContent dividers>
          <Grid container>
            {chart.analytic.map((itm, i) => {
              return (
                <Grid item xs={12} sm={6} md={4} className={classes.chartMap}>
                  <Typography variant="h6">{chart.que[i]}</Typography>
                  <Box className={classes.chart}>
                    <Chart
                      label={chart.que[i]}
                      labels={Object.keys(itm)}
