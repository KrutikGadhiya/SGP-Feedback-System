...
const Form = (props) => {
  ...
  const handleValidation = (fieldValues) => {
    ...
    if ("sem" in fieldValues)
      temp.sem =
        fieldValues.sem <= 8 && fieldValues.sem >= 1 ? "" : "between 1-5.";
    ...
    if ("feedbackQue" in fieldValues)
      temp.feedbackQue = fieldValues.feedbackQue
        ? ""
  ...
  const handleSubmit = async () => {
    ...
    if (!isFormValid()) {
      return dispatch(
        openSnack({ message: "Please Enter the Correct Values", type: "error" })
      );
    }
    if (
      !feedback.name ||
      ...
      return dispatch(
        openSnack({ message: "Please Add All the Fields", type: "error" })
      );
          ...
          >
            ...
            >
              ...
              {feedback.inst === "CSPIT"
                ...
                : feedback.inst === "DEPSTAR"
                ? instDEPSTAR.map((item, index) => {
                    return (
                      <MenuItem value={item} key={index}>
                        {item}
                      </MenuItem>
